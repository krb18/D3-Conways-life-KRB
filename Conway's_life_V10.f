...
2000 generations !                   { sets the number of generations to run the game for }

300 array-x-size !                                             { set initial x grid size }

300 array-y-size !                                             { set initial y grid size }
...
: xy_array_! array-y-size @ * + array @ + c! ;            { word to write and array. n x y }

: xy_array_@ array-y-size @ * + array @ + c@ ;             	 { word to read and array. x y }
...
: generation
  ...
  array-x-size @ array-y-size @ * 0 do
    i array-x-size @ mod i array-y-size @ / num_neighbours_wrap
    case
...
: generation_abs
  ...
  make_array new-array !
  array-x-size @ array-y-size @ * 0 do
    i array-x-size @ mod i array-y-size @ / num_neighbours_abs
    case
      0 of 0 new-array @ i array_! endof
      1 of 0 new-array @ i array_! endof
      4 of 0 new-array @ i array_! endof
...
: glider
  make_array array !                                      { create an initial empty array }
  1 4 98 xy_array_!                                    { this is a test setup of a glider }
  1 2 97 xy_array_!
  1 4 97 xy_array_!
  1 3 96 xy_array_!
  1 4 96 xy_array_!
...
: block
  make_array array !                                      { create an initial empty array }
  1 0 0 xy_array_!                                      { this is a test setup of a block }
  1 0 1 xy_array_!
  1 1 0 xy_array_!
  1 1 1 xy_array_!
  ;
