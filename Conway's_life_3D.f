...
variable array-z-size   { y dimension of array                                           }
...
: xyz_array_! array-x-size @ array-y-size @ * * swap array-x-size @ * + + array @ + c! ;            { word to write and array. n x y }

: xyz_array_@ array-x-size @ array-y-size @ * * swap array-x-size @ * + + array @ + c@ ;             	 { word to read and array. x y }
...
0 ti !
  ...
  currentz @ 2 + dup 3 - do
    ...
    currenty @ 2 + dup 3 - do
      currentx @ 2 + dup 3 - do
        I currentx @ = not J currenty @ = not or z @ currentz @ = not or if
          I J z @
        then
      loop
    loop
...
: num_neighbours
  0 neighbours !
  neighbour_loop 26 0 do
    dup 0 >= over array-z-size @ 1 - <= and if
      over dup 0 >= swap array-y-size @ 1 - <= and if
        rot dup 0 >= over array-x-size @ 1 - <= and if
          rot rot xyz_array_@ dup alive_dead
        else
          drop drop drop
        then
      else
...
: generation
  ...
  array-x-size @ array-y-size @ * array-z-size @ * 0 do
    i array-x-size @ array-y-size @ * mod array-x-size @ mod i array-x-size @ array-y-size @ * mod array-x-size @ / i array-x-size @ array-y-size @ * / num_neighbours
    case
