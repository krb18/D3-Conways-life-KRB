...
20000 generations !                   { sets the number of generations to run the game for }

300 bmp-x-size !                                               { set initial x grid size }

300 bmp-y-size !                                               { set initial y grid size }
...
: xy_array_! bmp-y-size @ * + array @ + c! ;            { word to write and array. n x y }

: xy_array_@ bmp-y-size @ * + array @ + c@ ;             	 { word to read and array. x y }
...
: make-test-file                                { Create a test file to read / write to  }
  s" C:\Users\tedje\Documents\Conway's Life\Test_File.dat" r/w create-file drop     \ Create the file
...
{ test writing an array to a file }
: testfilearray
  make-test-file
  test-file-size cr cr ." File Start Size = " d.
  bmp-x-size @ bmp-y-size @ * 0 do
    array @ I array_@ (.) test-file-id @ write-line drop
  loop
  test-file-size cr cr ." File End Size =   " d. cr cr
  close-test-file
  ;
...
: Pi
  make_array array !                                      { create an initial empty array }
  1 150 150 xy_array_!                           { this is a test setup of a pi heptomino }
  1 149 150 xy_array_!
  1 151 150 xy_array_!
  1 149 149 xy_array_!
  1 149 148 xy_array_!
...
: block
  make_array array !                                      { create an initial empty array }
  1 5 5 xy_array_!                                      { this is a test setup of a block }
  1 5 6 xy_array_!
  1 4 5 xy_array_!
  1 4 6 xy_array_!
  ;
...
: blinker
  make_array array !                                      { create an initial empty array }
  1 5 5 xy_array_!                                    { this is a test setup of a blinker }
  1 5 6 xy_array_!
  1 5 7 xy_array_!
  ;
